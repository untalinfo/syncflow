		...
		<div className={`group-card ${expanded ? "group-card--expanded" : ""}`}>
				...
				<div className="group-card__body">
					{totalCount === 0 ? (
						<div className="group-card__empty">
							...
							<p>
								No requests yet. Click "Add Request" to start building this
								group.
							</p>
						</div>
					) : (
						<ul className="group-card__items">
							{flatItems.map((item: SyncRequest) => (
								<li key={item.id} className="group-item">
									<span
										className={`group-item__dot group-item__dot--${item.status.toLowerCase()}`}
									/>
									<div className="group-item__info">
										...
										<span
											className={`group-item__badge group-item__badge--${item.status.toLowerCase()}`}
										>
											{item.status}
										</span>
									</div>
			...
			{showPicker && (
				<RequestPickerModal
					...
					onAssign={(rid) => {
						onAddRequest(storedGroup.id, rid);
						setShowPicker(false);
					}}
