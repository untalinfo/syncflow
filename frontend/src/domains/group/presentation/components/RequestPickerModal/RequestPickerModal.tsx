...
export const RequestPickerModal: React.FC<RequestPickerModalProps> = ({
  ...
  return (
    <Modal onClose={onClose}>
      <div className="picker-modal" role="dialog" aria-labelledby="picker-title">
        <h3 id="picker-title">Add Request to Group</h3>
        {unassigned.length === 0 ? (
          <p className="picker-empty">All requests are already in this group.</p>
        ) : (
          <ul className="picker-list">
            {unassigned.map((req) => (
              <li key={req.id} className="picker-item">
                <div className="picker-item__info">
                  <span className={`picker-dot picker-dot--${req.status.toLowerCase()}`} />
