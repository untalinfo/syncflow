...
import { useAppDependencies } from '../DependencyProvider';
...
export const RequestPage: React.FC<RequestPageProps> = ({ onNavigateHome }) => {
  ...
  return (
    <div className="app-container">
      <header className="header">
        <button className="btn-back" onClick={onNavigateHome}>🔙 Volver al Home</button>
          ...
          <div className="requests-grid">
            ...
            {requests.map((req, index) => (
              <div key={req.id || index} className={`request-card ${req.status.toLowerCase()}`}>
                ...
                <div className="card-footer">{new Date(req.createdAt).toLocaleString()}</div>
              </div>
            ))}
          </div>
        </section>
